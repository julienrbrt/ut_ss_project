...
public class BoardTest {
	...
	public void canPlaceTest() {
		//check whether or not the starting base can be placed in the middle
		assertTrue(board2p.canPlaceCheck(2, 2, true, 3, Color.REDDD, 0));
		board2p.addHome(2, 2);	//actually place the base now, also tests functionality of addHome
		//make sure there is something in the (2,2) spot
		assertFalse(board2p.canPlaceCheck(2, 2, false, 2, Color.REDDD, 0));
		//check invalid values
		assertFalse(board2p.canPlaceCheck(-1, 2, true, 2, Color.REDDD, 0));
		...
		//test surrounding-color checking
		assertTrue(board2p.canPlaceCheck(2, 3, true, 1, Color.BLUEE, 1));
		board3p.addHome(2, 2);
		//canPlace() does not actually place the piece, but it does reduce the ring counter
		assertTrue(board3p.canPlace(2, 3, false, 0, Color.YELLO, 0));
		//should return false, because player 0 only has 1 yellow ring of size 0
		// and it has already been counted down
		assertFalse(board3p.canPlace(2, 3, false, 0, Color.YELLO, 0));
		board4p.canPlace(2, 2, true, 1, Color.REDDD, 0);	//flip start boolean to false
		board4p.addHome(2, 2);
		//not next to starting base
		assertFalse(board4p.canPlaceCheck(3, 3, true, 1, Color.BLUEE, 1));
		board4p.canPlace(2, 3, false, 0, Color.BLUEE, 1);
		board4p.addRing(2, 3, false, 0, Color.BLUEE);
		//next to just placed piece
		assertTrue(board4p.canPlaceCheck(3, 3, true, 1, Color.BLUEE, 1));
